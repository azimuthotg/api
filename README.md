...
Production รันบน IIS + FastCGI (`web.config` → `wfastcgi.py`) โดยใช้ system Python `C:\Python312\python.exe` (ไม่มี venv)

| ค่า | Production |
|-----|-----------|
| Server | `202.29.55.217` (เข้าผ่าน RDP) |
| Project path | `C:\inetpub\wwwroot\NPUAPI\apiproject` |
| IIS site / App Pool | `apiproject` / `apiproject` |
| Host binding | `api.npu.ac.th` (`:80`, `:443`) เท่านั้น |

> หมายเหตุ: site ผูกเฉพาะ host `api.npu.ac.th` — การยิง `http://localhost/...` จะได้ IIS 404 ให้ทดสอบด้วย `-Headers @{Host='api.npu.ac.th'}` หรือเปิด URL จริง

### Deploy แบบ Git (วิธีปัจจุบัน)

โปรเจกต์ใช้ git-based deployment ผ่าน `deploy.ps1` (เลิกใช้ copy-paste แล้ว)

**Workflow:**
```
[dev]  แก้โค้ด → git commit → git push origin main
[prod] cd C:\inetpub\wwwroot\NPUAPI\apiproject
       powershell -ExecutionPolicy Bypass -File .\deploy.ps1
```

`deploy.ps1` ทำงานตามลำดับ:
1. ตรวจว่าไฟล์ที่ track ไม่มีการแก้ค้างบน server (ถ้ามีจะ abort)
2. `git pull origin main`
3. `python manage.py migrate --noinput`
4. `python manage.py collectstatic --noinput`
5. Recycle IIS app pool `apiproject` (wfastcgi โหลดโค้ดใหม่)

> ไฟล์ที่ git ไม่แตะบน prod (gitignored): `.env`, `*.json` (Google service account), `static/`, `db.sqlite3`, `code_deploy/`

### การตั้งค่าครั้งแรก (แปลง prod เดิมให้เป็น git repo)

ทำครั้งเดียวบน server ที่ deploy แบบ copy-paste มาก่อน:
```powershell
cd C:\inetpub\wwwroot\NPUAPI\apiproject
# 1. backup ก่อน
Compress-Archive -Path .\* -DestinationPath ..\apiproject_backup_$(Get-Date -Format yyyyMMdd_HHmmss).zip
# 2. เชื่อม git โดยไม่ทับไฟล์ (mixed reset)
git init
git symbolic-ref HEAD refs/heads/main
git config core.autocrlf true
git remote add origin https://github.com/azimuthotg/api.git
git fetch origin
git reset origin/main          # ไม่เขียนทับ working tree
git branch --set-upstream-to=origin/main main
# 3. ตรวจความต่าง แล้วค่อย sync เมื่อมั่นใจว่าไม่มี hotfix ตกหล่น
git status
git checkout -- .              # sync working tree ให้ตรง origin/main
```
