...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        moved = self._archive_old_rows(today, dry_run)
    ...
    def _archive_old_rows(self, today, dry_run):
        """ย้ายแถวที่เก่ากว่าวันนี้จากตารางสด -> คลัง ทีละชุด"""
        base_qs = ApiAccessLog.objects.filter(created_at__date__lt=today)
        total = base_qs.count()
        if total == 0:
        ...
        while True:
            ...
            pk_chunk = list(
                ApiAccessLog.objects.filter(created_at__date__lt=today)
                .order_by('created_at')
